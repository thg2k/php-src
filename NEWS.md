...
- PDO\_Firebird: Fixed bug #76452: Crash while parsing blob data in firebird\_fetch\_blob. (CVE-2021-21704) (cmb)

## 29 Apr 2021 - PHP v7.3.28
- Core: Fixed ./makedist wrt. to GH move. (cmb, Remi)
- Imap: Fixed bug #80710 (imap\_mail\_compose() header injection). (cmb, Stas)

## 04 Feb 2021 - PHP v7.3.27
- SOAP: Fixed bug #80672 (Null Dereference in SoapClient). (CVE-2021-21702) (cmb, Stas)

## 07 Jan 2021 - PHP v7.3.26
- Standard: Fixed bug #77423 (FILTER\_VALIDATE\_URL accepts URLs with invalid userinfo). (CVE-2020-7071) (cmb)
- Standard: Fixed bug #80457 (stream\_get\_contents() fails with maxlength=-1 or default). (bruno dot premont at restena dot lu)

## 26 Nov 2020 - PHP v7.3.25
...
- Intl: Fixed bug #80310 (ext-intl with icu4c 68.1: use of undeclared identifier 'TRUE'). (Alexander M. Turek)
...
- Curl: Fixed bug #79741 (curl\_setopt CURLOPT\_POSTFIELDS asserts on object with declared properties). (Nikita)
...
- Standard: Fixed bug #79817 (str\_replace() does not handle INDIRECT elements). (Nikita)
- Standard: Fixed bug #78008 (dns\_check\_record() always return true on Alpine). (Andy Postnikov)

## 09 Jul 2020 - PHP v7.3.20
...
- Opcache: Fixed bug #79535 (PHP crashes with specific opcache.optimization\_level). (Nikita)
...
## 14 May 2020 - PHP v7.3.18
- Core: Fixed bug #78875 (Long filenames cause OOM and temp files are not cleaned). (CVE-2019-11048) (cmb)
- Core: Fixed bug #78876 (Long variables in multipart/form-data cause OOM and temp files are not cleaned). (CVE-2019-11048) (cmb)
- Core: Fixed bug #79434 (PHP 7.3 and PHP-7.4 crash with NULL-pointer dereference on !CS constant). (Nikita)
- Core: Fixed bug #79477 (casting object into array creates references). (Nikita)
- Core: Fixed bug #79470 (PHP incompatible with 3rd party file system on demand). (cmb)
- Core: Fixed bug #78784 (Unable to interact with files inside a VFS for Git repository). (cmb)
...
- Phar: Fix bug #79503 (Memory leak on duplicate metadata). (cmb)
...
## 20 Feb 2020 - PHP v7.3.15
- Core: Fixed bug #71876 (Memory corruption htmlspecialchars(): charset \`\*' not supported). (Nikita)
...
- MBString: Fixed bug #79154 (mb\_convert\_encoding() can modify $from\_encoding). (cmb)
...
- SPL: Fixed bug #79151 (heap use after free caused by spl\_dllist\_it\_helper\_move\_forward). (Nikita)
...
- Mbstring: Fixed bug #79037 (global buffer-overflow in \`mbfl\_filt\_conv\_big5\_wchar\`). (CVE-2020-7060) (Nikita)
...
- Session: Fixed bug #79091 (heap use-after-free in session\_create\_id()). (cmb, Nikita)
- Shmop: Fixed bug #78538 (shmop memory leak). (cmb)
- Standard: Fixed bug #79099 (OOB read in php\_strip\_tags\_ex). (CVE-2020-7059). (cmb)
- Standard: Fixed bug #54298 (Using empty additional\_headers adding extraneous CRLF). (cmb)

## 18 Dec 2019 - PHP v7.3.13
- Bcmath: Fixed bug #78878 (Buffer underflow in bc\_shift\_addsub). (CVE-2019-11046). (cmb)
- Core: Fixed bug #78862 (link() silently truncates after a null byte on Windows). (CVE-2019-11044). (cmb)
- Core: Fixed bug #78863 (DirectoryIterator class silently truncates after a null byte). (CVE-2019-11045). (cmb)
- Core: Fixed bug #78943 (mail() may release string with refcount==1 twice). (CVE-2019-11049). (cmb)
- Core: Fixed bug #78787 (Segfault with trait overriding inherited private shadow property). (Nikita)
...
- EXIF: Fixed bug #78793 (Use-after-free in exif parsing under memory sanitizer). (CVE-2019-11050). (Nikita)
- EXIF: Fixed bug #78910 (Heap-buffer-overflow READ in exif). (CVE-2019-11047). (Nikita)
- GD: Fixed bug #78849 (GD build broken with -D SIGNED\_COMPARE\_SLOW). (cmb)
- MBString: Upgraded bundled Oniguruma to 6.9.4. (cmb)
- OPcache: Fixed potential ASLR related invalid opline handler issues. (cmb)
...
- Standard: Fixed bug #69100 (Bus error from stream\_copy\_to\_stream (file -> SSL stream) with invalid length). (Nikita)
...
- SPL: Fixed bug #77431 (openFile() silently truncates after a null byte). (cmb)
...
- Core: Fixed bug #74860 (Uncaught exceptions not being formatted properly when error\_log set to "syslog"). (Philip Prindeville)
...
- DBA: Fixed bug #75264 (compiler warnings emitted). (petk)
...
- Opcache: Fixed bug #77092 (array\_diff\_key() - segmentation fault). (Nikita)
...
- PDO\_DBlib: Implemented FR #69592 (allow 0-column rowsets to be skipped automatically). (fandrieu)
- PDO\_DBlib: Expose TDS version as \\PDO::DBLIB\_ATTR\_TDS\_VERSION attribute on \\PDO instance. (fandrieu)
- PDO\_DBlib: Treat DATETIME2 columns like DATETIME. (fandrieu)
- PDO\_DBlib: Fixed bug #74243 (allow locales.conf to drive datetime format). (fandrieu)
- PDO\_Firebird: Fixed bug #74462 (PDO\_Firebird returns only NULLs for results with boolean for FIREBIRD >= 3.0). (Dorin Marcoci)
- PDO\_OCI: Fixed bug #74631 (PDO\_PCO with PHP-FPM: OCI environment initialized before PHP-FPM sets it up). (Ingmar Runge)
- PDO SQLite: Add support for additional open flags
- pgsql: Added new error constants for pg\_result\_error(): PGSQL\_DIAG\_SCHEMA\_NAME, PGSQL\_DIAG\_TABLE\_NAME, PGSQL\_DIAG\_COLUMN\_NAME, PGSQL\_DIAG\_DATATYPE\_NAME, PGSQL\_DIAG\_CONSTRAINT\_NAME and PGSQL\_DIAG\_SEVERITY\_NONLOCALIZED. (Kalle)
- pgsql: Fixed bug #77047 (pg\_convert has a broken regex for the 'TIME WITHOUT TIMEZONE' data type). (Andy Gajetzki)
...
- readline: Added completion\_append\_character and completion\_suppress\_append options to readline\_info() if linked against libreadline. (krageon)
- Session: Fixed bug #74941 (session fails to start after having headers sent). (morozov)
- SimpleXML: Fixed bug #54973 (SimpleXML casts integers wrong). (Nikita)
- SimpleXML: Fixed bug #76712 (Assignment of empty string creates extraneous text node). (cmb)
...
- SOAP: Fixed bug #70469 (SoapClient generates E\_ERROR even if exceptions=1 is used). (Anton Artamonov)
- SOAP: Fixed bug #50675 (SoapClient can't handle object references correctly). (Cameron Porter)
- SOAP: Fixed bug #76348 (WSDL\_CACHE\_MEMORY causes Segmentation fault). (cmb)
- SOAP: Fixed bug #77141 (Signedness issue in SOAP when precision=-1). (cmb)
- SPL: Fixed bug #74977 (Appending AppendIterator leads to segfault). (Andrew Nester)
- SPL: Fixed bug #75173 (incorrect behavior of AppendIterator::append in foreach loop). (jhdxr)
- SPL: Fixed bug #74372 (autoloading file with syntax error uses next autoloader, may hide parse error). (Nikita)
- SPL: Fixed bug #75878 (RecursiveTreeIterator::setPostfix has wrong signature). (cmb)
...
- SQLite3: Updated bundled libsqlite to 3.24.0. (cmb)
- Standard: Added is\_countable() function. (Gabriel Caruso)
- Standard: Added support for the SameSite cookie directive, including an alternative signature for setcookie(), setrawcookie() and session\_set\_cookie\_params(). (Frederik Bosch, pmmaga)
- Standard: Remove superfluous warnings from inet\_ntop()/inet\_pton(). (daverandom)
- Standard: Fixed bug #75916 (DNS\_CAA record results contain garbage). (Mike, Philip Sharp)
- Standard: Fixed unserialize(), to disable creation of unsupported data structures through manually crafted strings. (Dmitry)
- Standard: Fixed bug #75409 (accept EFAULT in addition to ENOSYS as indicator that getrandom() is missing). (sarciszewski)
- Standard: Fixed bug #74719 (fopen() should accept NULL as context). (Alexander Holman)
- Standard: Fixed bug #69948 (path/domain are not sanitized in setcookie). (cmb)
- Standard: Fixed bug #75996 (incorrect url in header for mt\_rand). (tatarbj)
- Standard: Added hrtime() function, to get high resolution time. (welting)
- Standard: Fixed bug #48016 (stdClass::\_\_setState is not defined although var\_export() uses it). (Andrea)
- Standard: Fixed bug #76136 (stream\_socket\_get\_name should enclose IPv6 in brackets). (seliver)
- Standard: Fixed bug #76688 (Disallow excessive parameters after options array). (pmmaga)
- Standard: Fixed bug #76713 (Segmentation fault caused by property corruption). (Laruence)
- Standard: Fixed bug #76755 (setcookie does not accept "double" type for expire time). (Laruence)
- Standard: Fixed bug #76674 (improve array\_\* failure messages exposing what was passed instead of an array). (carusogabriel)
- Standard: Fixed bug #76803 (ftruncate changes file pointer). (Anatol)
- Standard: Fixed bug #76818 (Memory corruption and segfault). (Remi)
- Standard: Fixed bug #77081 (ftruncate() changes seek pointer in c mode). (cmb, Anatol)
- Testing: Implemented FR #62055 (Make run-tests.php support --CGI-- sections). (cmb)
- Tidy: Support using tidyp instead of tidy. (devnexen)
- Tidy: Fixed bug #74707 (Tidy has incorrect ReflectionFunction param counts for functions taking tidy). (Gabriel Caruso)
- Tidy: Fixed arginfo for tidy::\_\_construct(). (Tyson Andre)
- Tokenizer: Fixed bug #76437 (token\_get\_all with TOKEN\_PARSE flag fails to recognise close tag). (Laruence)
- Tokenizer: Fixed bug #75218 (Change remaining uncatchable fatal errors for parsing into ParseError). (Nikita)
- Tokenizer: Fixed bug #76538 (token\_get\_all with TOKEN\_PARSE flag fails to recognise close tag with newline). (Nikita)
- Tokenizer: Fixed bug #76991 (Incorrect tokenization of multiple invalid flexible heredoc strings). (Nikita)
- XML: Fixed bug #71592 (External entity processing never fails). (cmb)
- Zlib: Added zlib/level context option for compress.zlib wrapper. (Sara)

## 08 Nov 2018 - PHP v7.2.12
- Core: Fixed bug #76846 (Segfault in shutdown function after memory limit error). (Nikita)
- Core: Fixed bug #76946 (Cyclic reference in generator not detected). (Nikita)
- Core: Fixed bug #77035 (The phpize and ./configure create redundant .deps file). (Peter Kokot)
- Core: Fixed bug #77041 (buildconf should output error messages to stderr) (Mizunashi Mana)
- Date: Upgraded timelib to 2017.08. (Derick)
- Date: Fixed bug #75851 (Year component overflow with date formats "c", "o", "r" and "y"). (Adam Saponara)
- Date: Fixed bug #77007 (fractions in \`diff()\` are not correctly normalized). (Derick)
- FCGI: Fixed #76948 (Failed shutdown/reboot or end session in Windows). (Anatol)
- FCGI: Fixed bug #76954 (apache\_response\_headers removes last character from header name). (stodorovic)
- FTP: Fixed bug #76972 (Data truncation due to forceful ssl socket shutdown). (Manuel Mausz)
- intl: Fixed bug #76942 (U\_ARGUMENT\_TYPE\_MISMATCH). (anthrax at unixuser dot org)
- Reflection: Fixed bug #76936 (Objects cannot access their private attributes while handling reflection errors). (Nikita)
- Reflection: Fixed bug #66430 (ReflectionFunction::invoke does not invoke closure with object scope). (Nikita)
- Sodium: Some base64 outputs were truncated; this is not the case any more. (jedisct1)
- Sodium: block sizes >= 256 bytes are now supposed by sodium\_pad() even when an old version of libsodium has been installed. (jedisct1)
- Sodium: Fixed bug #77008 (sodium\_pad() could read (but not return nor write) uninitialized memory when trying to pad an empty input). (jedisct1)
- Standard: Fixed bug #76965 (INI\_SCANNER\_RAW doesn't strip trailing whitespace). (Pierrick)
- Tidy: Fixed bug #77027 (tidy::getOptDoc() not available on Windows). (cmb)
- XML: Fixed bug #30875 (xml\_parse\_into\_struct() does not resolve entities). (cmb)
- XML: Add support for getting SKIP\_TAGSTART and SKIP\_WHITE options. (cmb)
- XMLRPC: Fixed bug #75282 (xmlrpc\_encode\_request() crashes). (cmb)

## 11 Oct 2018 - PHP v7.2.11
- Core: Fixed bug #76800 (foreach inconsistent if array modified during loop). (Dmitry)
- Core: Fixed bug #76901 (method\_exists on SPL iterator passthrough method corrupts memory). (Nikita)
- CURL: Fixed bug #76480 (Use curl\_multi\_wait() so that timeouts are respected). (Pierrick)
- iconv: Fixed bug #66828 (iconv\_mime\_encode Q-encoding longer than it should be). (cmb)
- Opcache: Fixed bug #76832 (ZendOPcache.MemoryBase periodically deleted by the OS). (Anatol)
- Opcache: Fixed bug #76796 (Compile-time evaluation of disabled function in opcache causes segfault). (Nikita)
- POSIX: Fixed bug #75696 (posix\_getgrnam fails to print details of group). (cmb)
- Reflection: Fixed bug #74454 (Wrong exception being thrown when using ReflectionMethod). (cmb)
- Standard: Fixed bug #73457 (Wrong error message when fopen FTP wrapped fails to open data connection). (Ville Hukkamäki)
...
- Standard: Fixed bug #75533 (array\_reduce is slow when $carry is large array). (Manabu Matsui)
- XMLRPC: Fixed bug #76886 (Can't build xmlrpc with expat). (Thomas Petazzoni, cmb)
- Zlib: Fixed bug #75273 (php\_zlib\_inflate\_filter() may not update bytes\_consumed). (Martin Burke, cmb)

## 13 Sep 2018 - PHP v7.2.10
- Core: Fixed bug #76754 (parent private constant in extends class memory leak). (Laruence)
- Core: Fixed bug #72443 (Generate enabled extension). (petk)
- Core: Fixed bug #75797 (Memory leak when using class\_alias() in non-debug mode). (Massimiliano Braglia)
- Apache2: Fixed bug #76582 (Apache bucket brigade sometimes becomes invalid). (stas)
- Bz2: Fixed arginfo for bzcompress. (Tyson Andre)
- gettext: Fixed bug #76517 (incorrect restoring of LDFLAGS). (sji)
- iconv: Fixed bug #68180 (iconv\_mime\_decode can return extra characters in a header). (cmb)
- iconv: Fixed bug #63839 (iconv\_mime\_decode\_headers function is skipping headers). (cmb)
- iconv: Fixed bug #60494 (iconv\_mime\_decode does ignore special characters). (cmb)
- iconv: Fixed bug #55146 (iconv\_mime\_decode\_headers() skips some headers). (cmb)
- intl: Fixed bug #74484 (MessageFormatter::formatMessage memory corruption with 11+ named placeholders). (Anatol)
- libxml: Fixed bug #76777 ("public id" parameter of libxml\_set\_external\_entity\_loader callback undefined). (Ville Hukkamäki)
- mbstring: Fixed bug #76704 (mb\_detect\_order return value varies based on argument type). (cmb)
- Opcache: Fixed bug #76747 (Opcache treats path containing "test.pharma.tld" as a phar file). (Laruence)
- OpenSSL: Fixed bug #76705 (unusable ssl => peer\_fingerprint in stream\_context\_create()). (Jakub Zelenka)
- phpdbg: Fixed bug #76595 (phpdbg man page contains outdated information). (Kevin Abel)
- SPL: Fixed bug #68825 (Exception in DirectoryIterator::getLinkTarget()). (cmb)
- SPL: Fixed bug #68175 (RegexIterator pregFlags are NULL instead of 0). (Tim Siebels)
- Standard: Fixed bug #76778 (array\_reduce leaks memory if callback throws exception). (cmb)
- zlib: Fixed bug #65988 (Zlib version check fails when an include/zlib/ style dir is passed to the --with-zlib configure option). (Jay Bonci)
- zlib: Fixed bug #76709 (Minimal required zlib library is 1.2.0.4). (petk)

## 16 Aug 2018 - PHP v7.2.9
- Calendar: Fixed bug #52974 (jewish.c: compile error under Windows with GBK charset). (cmb)
- Filter: Fixed bug #76366 (References in sub-array for filtering breaks the filter). (ZiHang Gao)
- PDO\_Firebird: Fixed bug #76488 (Memory leak when fetching a BLOB field). (Simonov Denis)
- PDO\_PgSQL: Fixed bug #75402 (Possible Memory Leak using PDO::CURSOR\_SCROLL option). (Anatol)
- SQLite3: Fixed #76665 (SQLite3Stmt::bindValue() with SQLITE3\_FLOAT doesn't juggle). (cmb)
- Standard: Fixed bug #73817 (Incorrect entries in get\_html\_translation\_table). (cmb)
- Standard: Fixed bug #68553 (array\_column: null values in $index\_key become incrementing keys in result). (Laruence)
- Standard: Fixed bug #76643 (Segmentation fault when using \`output\_add\_rewrite\_var\`). (cmb)
- Zip: Fixed bug #76524 (ZipArchive memory leak (OVERWRITE flag and empty archive)). (Timur Ibragimov)

## 19 Jul 2018 - PHP v7.2.8
- Core: Fixed bug #76534 (PHP hangs on 'illegal string offset on string references with an error handler). (Laruence)
- Core: Fixed bug #76520 (Object creation leaks memory when executed over HTTP). (Nikita)
- Core: Fixed bug #76502 (Chain of mixed exceptions and errors does not serialize properly). (Nikita)
- Date: Fixed bug #76462 (Undefined property: DateInterval::$f). (Anatol)
- EXIF: Fixed bug #76409 (heap use after free in \_php\_stream\_free). (cmb)
- EXIF: Fixed bug #76423 (Int Overflow lead to Heap OverFlow in exif\_thumbnail\_extract of exif.c). (Stas)
- EXIF: Fixed bug #76557 (heap-buffer-overflow (READ of size 48) while reading exif data). (Stas)
...
- Opcache: Fixed bug #76477 (Opcache causes empty return value). (Nikita, Laruence)
...
- Reflection: Fixed bug #76536 (PHP crashes with core dump when throwing exception in error handler). (Laruence)
- Reflection: Fixed bug #75231 (ReflectionProperty#getValue() incorrectly works with inherited classes). (Nikita)
- Standard: Fixed bug #76505 (array\_merge\_recursive() is duplicating sub-array keys). (Laruence)
- Standard: Fixed bug #71848 (getimagesize with $imageinfo returns false). (cmb)
- Win32: Fixed bug #76459 (windows linkinfo lacks openbasedir check). (Anatol)
- ZIP: Fixed bug #76461 (OPSYS\_Z\_CPM defined instead of OPSYS\_CPM). (Dennis Birkholz, Remi)

## 07 Jun 2018 - PHP v7.2.7
- Core: Fixed bug #76337 (segfault when opcache enabled + extension use zend\_register\_class\_alias). (xKhorasan)
- CLI Server: Fixed bug #76333 (PHP built-in server does not find files if root path contains special characters). (Anatol)
- OpenSSL: Fixed bug #76296 (openssl\_pkey\_get\_public does not respect open\_basedir). (Erik Lax, Jakub Zelenka)
- OpenSSL: Fixed bug #76174 (openssl extension fails to build with LibreSSL 2.7). (Jakub Zelenka)
- SPL: Fixed bug #76367 (NoRewindIterator segfault 11). (Laruence)
- Standard: Fixed bug #76410 (SIGV in zend\_mm\_alloc\_small). (Laruence)
- Standard: Fixed bug #76335 ("link(): Bad file descriptor" with non-ASCII path). (Anatol)

## 24 May 2018 - PHP v7.2.6
- EXIF: Fixed bug #76164 (exif\_read\_data zend\_mm\_heap corrupted). (cmb)
...
- Opcache: Fixed bug #76205 (PHP-FPM sporadic crash when running Infinitewp). (Dmitry)
- Opcache: Fixed bug #76275 (Assertion failure in file cache when unserializing empty try\_catch\_array). (Nikita)
- Opcache: Fixed bug #76281 (Opcache causes incorrect "undefined variable" errors). (Nikita)
- Reflection: Fixed arginfo of array\_replace(\_recursive) and array\_merge(\_recursive). (carusogabriel)
- Session: Fixed bug #74892 (Url Rewriting (trans\_sid) not working on urls that start with "#"). (Andrew Nester)

## 26 Apr 2018 - PHP v7.2.5
- Core: Fixed bug #75722 (Convert valgrind detection to configure option). (Michael Heimpold)
- Date: Fixed bug #76131 (mismatch arginfo for date\_create). (carusogabriel)
- Exif: Fixed bug #76130 (Heap Buffer Overflow (READ: 1786) in exif\_iif\_add\_value). (Stas)
- FPM: Fixed bug #68440 (ERROR: failed to reload: execvp() failed: Argument list too long). (Jacob Hipps)
- FPM: Fixed incorrect write to getenv result in FPM reload. (Jakub Zelenka)
- GD: Fixed bug #52070 (imagedashedline() - dashed line sometimes is not visible). (cmb)
- iconv: Fixed bug #76249 (stream filter convert.iconv leads to infinite loop on invalid sequence). (Stas)
- intl: Fixed bug #76153 (Intl compilation fails with icu4c 61.1). (Anatol)
- ldap: Fixed bug #76248 (Malicious LDAP-Server Response causes Crash). (Stas)
- mbstring: Fixed bug #75944 (Wrong cp1251 detection). (dmk001)
- mbstring: Fixed bug #76113 (mbstring does not build with Oniguruma 6.8.1). (chrullrich, cmb)
- ODBC: Fixed bug #76088 (ODBC functions are not available by default on Windows). (cmb)
- Opcache: Fixed bug #76094 (Access violation when using opcache). (Laruence)
- Phar: Fixed bug #76129 (fix for CVE-2018-5712 may not be complete). (Stas)
- phpdbg: Fixed bug #76143 (Memory corruption: arbitrary NUL overwrite). (Laruence)
- SPL: Fixed bug #76131 (mismatch arginfo for splarray constructor). (carusogabriel)
- standard: Fixed bug #74139 (mail.add\_x\_header default inconsistent with docs). (cmb)
- standard: Fixed bug #75996 (incorrect url in header for mt\_rand). (tatarbj)

## 29 Mar 2018 - PHP v7.2.4
- Core: Fixed bug #76025 (Segfault while throwing exception in error\_handler). (Dmitry, Laruence)
- Core: Fixed bug #76044 ('date: illegal option -- -' in ./configure on FreeBSD). (Anatol)
...
- GD: Fixed bug #76041 (null pointer access crashed php). (cmb)
...
- Standard: Fixed bug #75961 (Strange references behavior). (Laruence)
- Standard: Fixed some arginfos. (carusogabriel)
- Standard: Fixed bug #76068 (parse\_ini\_string fails to parse "[foo]\\nbar=1|>baz" with segfault). (Anatol)

## 01 Mar 2018 - PHP v7.2.3
- Core: Fixed bug #75864 ("stream\_isatty" returns wrong value on s390x). (Sam Ding)
- Apache2Handler: Fixed bug #75882 (a simple way for segfaults in threadsafe php just with configuration). (Anatol)
- Date: Fixed bug #75857 (Timezone gets truncated when formatted). (carusogabriel)
- Date: Fixed bug #75928 (Argument 2 for \`DateTimeZone::listIdentifiers()\` should accept \`null\`). (Pedro Lacerda)
- Date: Fixed bug #68406 (calling var\_dump on a DateTimeZone object modifies it). (jhdxr)
- LDAP: Fixed bug #49876 (Fix LDAP path lookup on 64-bit distros). (dzuelke)
- libxml2: Fixed bug #75871 (use pkg-config where available). (pmmaga)
- PGSQL: Fixed bug #75838 (Memory leak in pg\_escape\_bytea()). (ard\_1 at mail dot ru)
- Phar: Fixed bug #54289 (Phar::extractTo() does not accept specific directories to be extracted). (bishop)
- Phar: Fixed bug #65414 (deal with leading slash while adding files correctly). (bishopb)
- Phar: Fixed bug #65414 (deal with leading slash when adding files correctly). (bishopb)
- ODBC: Fixed bug #73725 (Unable to retrieve value of varchar(max) type). (Anatol)
- Opcache: Fixed bug #75729 (opcache segfault when installing Bitrix). (Nikita)
- Opcache: Fixed bug #75893 (file\_get\_contents $http\_response\_header variable bugged with opcache). (Nikita)
- Opcache: Fixed bug #75938 (Modulus value not stored in variable). (Nikita)
- SPL: Fixed bug #74519 (strange behavior of AppendIterator). (jhdxr)
- Standard: Fixed bug #75916 (DNS\_CAA record results contain garbage). (Mike, Philip Sharp)
- Standard: Fixed bug #75981 (Prevent reading beyond buffer start in http wrapper). (Stas)

## 01 Feb 2018 - PHP v7.2.2
- Core: Fixed bug #75742 (potential memleak in internal classes's static members). (Laruence)
- Core: Fixed bug #75679 (Path 260 character problem). (Anatol)
- Core: Fixed bug #75614 (Some non-portable == in shell scripts). (jdolecek)
- Core: Fixed bug #75786 (segfault when using spread operator on generator passed by reference). (Nikita)
- Core: Fixed bug #75799 (arg of get\_defined\_functions is optional). (carusogabriel)
- Core: Fixed bug #75396 (Exit inside generator finally results in fatal error). (Nikita)
...
- Opcache: Fixed bug #75720 (File cache not populated after SHM runs full). (Dmitry)
- Opcache: Fixed bug #75687 (var 8 (TMP) has array key type but not value type). (Nikita, Laruence)
- Opcache: Fixed bug #75698 (Using @ crashes php7.2-fpm). (Nikita)
- Opcache: Fixed bug #75579 (Interned strings buffer overflow may cause crash). (Dmitry)
- PDO: Fixed bug #75616 (PDO extension doesn't allow to be built shared on Darwin). (jdolecek)
- PDO MySQL: Fixed bug #75615 (PDO Mysql module can't be built as module). (jdolecek)
...
- SOAP: Fixed bug #70469 (SoapClient generates E\_ERROR even if exceptions=1 is used). (Anton Artamonov)
- SOAP: Fixed bug #75502 (Segmentation fault in zend\_string\_release). (Nikita)
- SPL: Fixed bug #75717 (RecursiveArrayIterator does not traverse arrays by reference). (Nikita)
- SPL: Fixed bug #75242 (RecursiveArrayIterator doesn't have constants from parent class). (Nikita)
- SPL: Fixed bug #73209 (RecursiveArrayIterator does not iterate object properties). (Nikita)
- Zip: Display headers (buildtime) and library (runtime) versions in phpinfo (with libzip >= 1.3.1). (Remi)

## 04 Jan 2018 - PHP v7.2.1
- Core: Fixed bug #75573 (Segmentation fault in 7.1.12 and 7.0.26). (Laruence)
- Core: Fixed bug #75384 (PHP seems incompatible with OneDrive files on demand). (Anatol)
- Core: Fixed bug #75525 (Access Violation in vcruntime140.dll). (Anatol)
- Core: Fixed bug #74862 (Unable to clone instance when private \_\_clone defined). (Daniel Ciochiu)
- Core: Fixed bug #75074 (php-process crash when is\_file() is used with strings longer 260 chars). (Anatol)
- Core: Fixed bug #69727 (Remove timestamps from build to make it reproducible). (jelle van der Waa)
- CLI server: Fixed bug #73830 (Directory does not exist). (Anatol)
...
- Opcache: Fixed bug #75608 ("Narrowing occurred during type inference" error). (Laruence, Dmitry)
- Opcache: Fixed bug #75579 (Interned strings buffer overflow may cause crash). (Dmitry)
- Opcache: Fixed bug #75570 ("Narrowing occurred during type inference" error). (Dmitry)
- Opcache: Fixed bug #75681 (Warning: Narrowing occurred during type inference, specific case). (Nikita)
- Opcache: Fixed bug #75556 (Invalid opcode 138/1/1). (Laruence)
...
- Standard: Fixed bug #75511 (fread not free unused buffer). (Laruence)
- Standard: Fixed bug #75514 (mt\_rand returns value outside [$min,$max]+ on 32-bit) (Remi)
- Standard: Fixed bug #75535 (Inappropriately parsing HTTP response leads to PHP segment fault). (Nikita)
- Standard: Fixed bug #75409 (accept EFAULT in addition to ENOSYS as indicator that getrandom() is missing). (sarciszewski)
- Standard: Fixed bug #73124 (php\_ini\_scanned\_files() not reporting correctly). (John Stevenson)
- Standard: Fixed bug #75574 (putenv does not work properly if parameter contains non-ASCII unicode character). (Anatol)
- Zip: Fixed bug #75540 (Segfault with libzip 1.3.1). (Remi)

## 30 Nov 2017 - PHP v7.2.0
- BCMath: Fixed bug #46564 (bcmod truncates fractionals). (liborm85)
- CLI: Fixed bug #74849 (Process is started as interactive shell in PhpStorm). (Anatol)
- CLI: Fixed bug #74979 (Interactive shell opening instead of script execution with -f flag). (Anatol)
- CLI server: Fixed bug #60471 (Random "Invalid request (unexpected EOF)" using a router script). (SammyK)
- Core: Added ZEND\_COUNT, ZEND\_GET\_CLASS, ZEND\_GET\_CALLED\_CLASS, ZEND\_GET\_TYPE, ZEND\_FUNC\_NUM\_ARGS, ZEND\_FUNC\_GET\_ARGS instructions, to implement corresponding builtin functions. (Dmitry)
- Core: "Countable" interface is moved from SPL to Core. (Dmitry)
- Core: Added ZEND\_IN\_ARRAY instruction, implementing optimized in\_array() builtin function, through hash lookup in flipped array. (Dmitry)
- Core: Removed IS\_TYPE\_IMMUTABLE (it's the same as COPYABLE & !REFCOUNTED). (Dmitry)
- Core: Removed the sql.safe\_mode directive. (Kalle)
- Core: Removed support for Netware. (Kalle)
- Core: Renamed ReflectionClass::isIterateable() to ReflectionClass::isIterable() (alias original name for BC). (Sara)
- Core: Fixed bug #54535 (WSA cleanup executes before MSHUTDOWN). (Kalle)
- Core: Implemented FR #69791 (Disallow mail header injections by extra headers) (Yasuo)
- Core: Implemented FR #49806 (proc\_nice() for Windows). (Kalle)
- Core: Fix pthreads detection when cross-compiling (ffontaine)
- Core: Fixed memory leaks caused by exceptions thrown from destructors. (Bob, Dmitry).
- Core: Fixed bug #73215 (uniqid() should use better random source). (Yasuo)
- Core: Implemented FR #72768 (Add ENABLE\_VIRTUAL\_TERMINAL\_PROCESSING flag for php.exe). (Michele Locati)
- Core: Implemented "Convert numeric keys in object/array casts" RFC, fixes bugs #53838, #61655, #66173, #70925, #72254, etc. (Andrea)
- Core: Implemented "Deprecate and Remove Bareword (Unquoted) Strings" RFC. (Rowan Collins)
- Core: Raised minimum supported Windows versions to Windows 7/Server 2008 R2. (Anatol)
- Core: Implemented minor optimization in array\_keys/array\_values(). (Sara)
- Core: Added PHP\_OS\_FAMILY constant to determine on which OS we are. (Jan Altensen)
- Core: Fixed bug #73987 (Method compatibility check looks to original definition and not parent). (pmmaga)
- Core: Fixed bug #73991 (JSON\_OBJECT\_AS\_ARRAY not respected). (Sara)
- Core: Fixed bug #74053 (Corrupted class entries on shutdown when a destructor spawns another object). (jim at commercebyte dot com)
- Core: Fixed bug #73971 (Filename got limited to MAX\_PATH on Win32 when scan directory). (Anatol)
- Core: Fixed bug #72359, bug #72451, bug #73706, bug #71115 and others related to interned strings handling in TS builds. (Anatol, Dmitry)
- Core: Implemented "Trailing Commas In List Syntax" RFC for group use lists only. (Sammy Kaye Powers)
- Core: Fixed bug #74269 (It's possible to override trait property with different loosely-equal value). (pmmaga)
- Core: Fixed bug #61970 (Restraining \_\_construct() access level in subclass gives a fatal error). (pmmaga)
- Core: Fixed bug #63384 (Cannot override an abstract method with an abstract method). (pmmaga, wes)
- Core: Fixed bug #74607 (Traits enforce different inheritance rules). (pmmaga)
- Core: Fixed misparsing of abstract unix domain socket names. (Sara)
- Core: Change PHP\_OS\_FAMILY value from "OSX" to "Darwin". (Sebastian, Kalle)
- Core: Allow loading PHP/Zend extensions by name in ini files (extension=<name>). (francois at tekwire dot net)
- Core: Added object type annotation. (brzuchal)
- Core: Fixed bug #74815 (crash with a combination of INI entries at startup). (Anatol)
- Core: Fixed bug #74836 (isset on zero-prefixed numeric indexes in array broken). (Dmitry)
- Core: Added new VM instuctions ISSET\_ISEMPTY\_CV and UNSET\_CV. Previously they were implemented as ISSET\_ISEMPTY\_VAR and UNSET\_VAR variants with ZEND\_QUICK\_SET flag. (Nikita, Dmitry)
- Core: Fixed bug #49649 (unserialize() doesn't handle changes in property visibility). (pmmaga)
- Core: Fixed #74866 (extension\_dir = "./ext" now use current directory for base). (Francois Laupretre)
- Core: Implemented FR #74963 (Improved error message on fetching property of non-object). (Laruence)
- Core: Fixed Bug #75142 (buildcheck.sh check for autoconf version needs to be updated for v2.64). (zizzy at zizzy dot net, Remi)
- Core: Fixed bug #74878 (Data race in ZTS builds). (Nikita, Dmitry)
- Core: Fixed bug #75515 ("stream\_copy\_to\_stream" doesn't stream anymore). (Sara)
- cURL: Fixed bug #75093 (OpenSSL support not detected). (Remi)
- cURL: Better fix for #74125 (use pkg-config instead of curl-config). (Remi)
- Date: Fixed bug #55407 (Impossible to prototype DateTime::createFromFormat). (kelunik)
- Date: Implemented FR #71520 (Adding the DateTime constants to the DateTimeInterface interface). (Majkl578)
- Date: Fixed bug #75149 (redefinition of typedefs ttinfo and t1info). (Remi)
- Date: Fixed bug #75222 (DateInterval microseconds property always 0). (jhdxr)
- Dba: Fixed bug #72885 (flatfile: dba\_fetch() fails to read replaced entry). (Anatol)
- DOM: Implement #74837 (Implement Countable for DomNodeList and DOMNamedNodeMap). (Andreas Treichel)
- EXIF: Added support for vendor specific tags for the following formats: Samsung, DJI, Panasonic, Sony, Pentax, Minolta, Sigma/Foveon, AGFA,
- EXIF: Fixed bug #72682 (exif\_read\_data() fails to read all data for some images). (Kalle)
- EXIF: Fixed bug #71534 (Type confusion in exif\_read\_data() leading to heap overflow in debug mode). (hlt99 at blinkenshell dot org, Kalle)
- EXIF: Fixed bug #68547 (Exif Header component value check error). (sjh21a at gmail dot com, Kalle)
- EXIF: Fixed bug #66443 (Corrupt EXIF header: maximum directory nesting level reached for some cameras). (Kalle)
- EXIF: Fixed Redhat bug #1362571 (PHP not returning full results for exif\_read\_data function). (Kalle)
- EXIF: Implemented #65187 (exif\_read\_data/thumbnail: add support for stream resource). (Kalle)
- EXIF: Deprecated the read\_exif\_data() alias. (Kalle)
- EXIF: Fixed bug #74428 (exif\_read\_data(): "Illegal IFD size" warning occurs with correct exif format). (bradpiccho at gmail dot com, Kalle)
- EXIF: Fixed bug #72819 (EXIF thumbnails not read anymore). (Kalle)
- EXIF: Fixed bug #62523 (php crashes with segfault when exif\_read\_data called). (Kalle)
- EXIF: Fixed bug #50660 (exif\_read\_data(): Illegal IFD offset (works fine with other exif readers). (skinny dot bravo at gmail dot com, Kalle)
- Fileinfo: Upgrade bundled libmagic to 5.31. (Anatol)
- FPM: Configuration to limit fpm slow log trace callers. (Sannis)
- FPM: Fixed bug #75212 (php\_value acts like php\_admin\_value). (Remi)
- FTP: Implement MLSD for structured listing of directories. (blar)
- FTP: Added ftp\_append() function. (blar)
- GD: Implemented imageresolution as getter and setter (Christoph)
- GD: Fixed bug #74744 (gd.h: stdarg.h include missing for va\_list use in gdErrorMethod). (rainer dot jung at kippdata dot de, cmb)
- GD: Fixed bug #75111 (Memory disclosure or DoS via crafted .bmp image). (cmb)
- GMP: Fixed bug #70896 (gmp\_fact() silently ignores non-integer input). (Sara)
- Hash: Changed HashContext from resource to object. (Rouven Weßling, Sara)
- Hash: Disallowed usage of non-cryptographic hash functions with HMAC and PBKDF2. (Andrey Andreev, Nikita)
- Hash: Fixed Bug #75284 (sha3 is not supported on bigendian machine). (Remi)
- IMAP: Fixed bug #72324 (imap\_mailboxmsginfo() return wrong size). (ronaldpoon at udomain dot com dot hk, Kalle)
- Intl: Fixed bug #63790 (test using Spoofchecker which may be unavailable). (Sara)
- Intl: Fixed bug #75378 ([REGRESSION] IntlDateFormatter::parse() does not change $position argument). (Laruence)
- JSON: Add JSON\_INVALID\_UTF8\_IGNORE and JSON\_INVALID\_UTF8\_SUBSTITUTE options for json\_encode and json\_decode to ignore or replace invalid UTF-8 byte sequences - it addresses request #65082. (Jakub Zelenka)
- JSON: Fixed bug #75185 (Buffer overflow in json\_decode() with JSON\_INVALID\_UTF8\_IGNORE or JSON\_INVALID). (Jakub Zelenka)
- JSON: Fixed bug #68567 (JSON\_PARTIAL\_OUTPUT\_ON\_ERROR can result in JSON with null key). (Jakub Zelenka)
- LDAP: Implemented FR #69445 (Support for LDAP EXOP operations)
- LDAP: Fixed support for LDAP\_OPT\_SERVER\_CONTROLS and LDAP\_OPT\_CLIENT\_CONTROLS in ldap\_get\_option
- LDAP: Fixed passing an empty array to ldap\_set\_option for client or server controls.
- Mbstring: Implemented request #66024 (mb\_chr() and mb\_ord()). (Masakielastic, Yasuo)
- Mbstring: Implemented request #65081 (mb\_scrub()). (Masakielastic, Yasuo)
- Mbstring: Implemented request #69086 (enhancement for mb\_convert\_encoding() that handles multibyte replacement char nicely). (Masakielastic, Yasuo)
- Mbstring: Added array input support to mb\_convert\_encoding(). (Yasuo)
- Mbstring: Added array input support to mb\_check\_encoding(). (Yasuo)
- Mbstring: Fixed bug #69079 (enhancement for mb\_substitute\_character). (masakielastic)
- Mbstring: Update to oniguruma version 6.3.0. (Remi)
- Mbstring: Fixed bug #69267 (mb\_strtolower fails on titlecase characters). (Nikita)
- Mcrypt: The deprecated mcrypt extension has been moved to PECL. (leigh)
...
- Opcache: Fixed incorect constant conditional jump elimination. (Dmitry)
- Opcache: Fixed bug #75230 (Invalid opcode 49/1/8 using opcache). (Laruence)
- Opcache: Fixed bug (assertion fails with extended info generated). (Laruence)
- Opcache: Fixed bug (Phi sources removel). (Laruence)
- Opcache: Fixed bug #75370 (Webserver hangs on valid PHP text). (Laruence)
- Opcache: Fixed bug #75357 (segfault loading WordPress wp-admin). (Laruence)
- OpenSSL: Use TLS\_ANY for default ssl:// and tls:// negotiation. (kelunik)
- OpenSSL: Fix leak in openssl\_spki\_new(). (jelle at vdwaa dot nl)
- OpenSSL: Added openssl\_pkcs7\_read() and pk7 parameter to openssl\_pkcs7\_verify(). (jelle at vdwaa dot nl)
- OpenSSL: Add ssl security\_level stream option to support OpenSSL security levels. (Jakub Zelenka).
- OpenSSL: Allow setting SNI cert and private key in separate files. (Jakub Zelenka)
- OpenSSL: Fixed bug #74903 (openssl\_pkcs7\_encrypt() uses different EOL than before). (Anatol)
- OpenSSL: Automatically load OpenSSL configuration file. (Jakub Zelenka)
- PCRE: Added support for PCRE JIT fast path API. (dmitry)
- PCRE: Fixed bug #61780 (Inconsistent PCRE captures in match results). (cmb)
- PCRE: Fixed bug #74873 (Minor BC break: PCRE\_JIT changes output of preg\_match()). (Dmitry)
- PCRE: Fixed bug #75089 (preg\_grep() is not reporting PREG\_BAD\_UTF8\_ERROR after first input string). (Dmitry)
- PCRE: Fixed bug #75223 (PCRE JIT broken in 7.2). (Dmitry)
- PCRE: Fixed bug #75285 (Broken build when system libpcre don't have jit support). (Remi)
- phar: Fixed bug #74196 (phar does not correctly handle names containing dots). (mhagstrand)
- PDO: Add "Sent SQL" to debug dump for emulated prepares. (Adam Baratz)
- PDO: Add parameter types for national character set strings. (Adam Baratz)
- PDO\_DBlib: Fixed bug #73234 (Emulated statements let value dictate parameter type). (Adam Baratz)
- PDO\_DBlib: Fixed bug #73396 (bigint columns are returned as strings). (Adam Baratz)
- PDO\_DBlib: Expose DB-Library version as \\PDO::DBLIB\_ATTR\_VERSION attribute on \\PDO instance. (Adam Baratz)
- PDO\_DBlib: Add test coverage for bug #72969. (Jeff Farr)
- PDO\_OCI: Fixed Bug #74537 (Align --with-pdo-oci configure option with --with-oci8 syntax). (Tianfang Yang)
- PDO\_Sqlite: Switch to sqlite3\_prepare\_v2() and sqlite3\_close\_v2() functions (rasmus)
- PHPDBG: Added extended\_value to opcode dump output. (Sara)
- Session: Fixed bug #73461 (Prohibit session save handler recursion). (Yasuo)
- Session: PR #2233 Removed register\_globals related code and "!" can be used as $\_SESSION key name. (Yasuo)
- Session: Improved bug #73100 fix. 'user' save handler can only be set by session\_set\_save\_handler()
- Session: Fixed bug #74514 (5 session functions incorrectly warn when calling in read-only/getter mode). (Yasuo)
- Session: Fixed bug #74936 (session\_cache\_expire/cache\_limiter/save\_path() trigger a warning in read mode). (morozov)
- Session: Fixed bug #74941 (session fails to start after having headers sent). (morozov)
- Sodium: New cryptographic extension
- Sodium: Added missing bindings for libsodium > 1.0.13. (Frank)
- SPL: Fixed bug #71412 (Incorrect arginfo for ArrayIterator::\_\_construct). (tysonandre775 at hotmail dot com)
- SPL: Added spl\_object\_id(). (Tyson Andre)
- SQLite3: Implement writing to blobs. (bohwaz at github dot com)
- SQLite3: Update to Sqlite 3.20.1. (cmb)
- Standard: Fixed bug #69442 (closing of fd incorrect when PTS enabled). (jaytaph)
- Standard: Fixed bug #74300 (unserialize accepts two plus/minus signs for float number exponent part). (xKerman)
- Standard: Compatibility with libargon2 versions 20161029 and 20160821. (charlesportwoodii at erianna dot com)
- Standard: Fixed Bug #74737 (mysqli\_get\_client\_info reflection info). (mhagstrand at gmail dot com)
- Standard: Add support for extension name as argument to dl(). (francois at tekwire dot net)
- Standard: Fixed bug #74851 (uniqid() without more\_entropy performs badly). (Emmanuel Dreyfus)
- Standard: Fixed bug #74103 (heap-use-after-free when unserializing invalid array size). (Nikita)
- Standard: Fixed bug #75054 (A Denial of Service Vulnerability was found when performing deserialization). (Nikita)
- Standard: Fixed bug #75170 (mt\_rand() bias on 64-bit machines). (Nikita)
- Standard: Fixed bug #75221 (Argon2i always throws NUL at the end). (cmb)
- Streams: Default ssl/single\_dh\_use and ssl/honor\_cipher\_order to true. (kelunik)
- XML: Moved utf8\_encode() and utf8\_decode() to the Standard extension. (Andrea)
- XMLRPC: Use Zend MM for allocation in bundled libxmlrpc (Joe)
- ZIP: Add support for encrypted archives. (Remi)
- ZIP: Use of bundled libzip is deprecated, --with-libzip option is recommended. (Remi)
- ZIP: Fixed Bug #73803 (Reflection of ZipArchive does not show public properties). (Remi)
- ZIP: ZipArchive implements countable, added ZipArchive::count() method. (Remi)
- ZIP: Fix segfault in php\_stream\_context\_get\_option call. (Remi)
- ZIP: Fixed bug #75143 (new method setEncryptionName() seems not to exist in ZipArchive). (Anatol)
- zlib: Expose inflate\_get\_status() and inflate\_get\_read\_len() functions. (Matthew Trescott)
